 ...
 */
public class ConfirmReplies extends AbstractAction {

    @Override
    public void execute(String[] args) throws RestApiException {
        if (args.length < 1) {
            System.out.println("Please provide at least one reply ID as a command line argument.");
            System.exit(1);
        }

        try {
            Settings.CLIENT.messaging().confirmReplies(Arrays.asList(args));
