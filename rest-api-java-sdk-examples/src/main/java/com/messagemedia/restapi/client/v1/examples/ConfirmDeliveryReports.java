 ...
 */
public class ConfirmDeliveryReports {
    ...
    public static void main(String[] args) throws RestApiException {
        ...
        // put in your delivery report ID(s) here
        final List<String> deliveryReportIds = Arrays.asList("<your delivery report ID>", "<your next ID>");

        final RestApiClient restApiClient = RestApiClientBuilder.newBuilder(apiKey, secretKey).build();
        try {
            restApiClient.messaging().confirmDeliveryReports(deliveryReportIds);
            System.out.println("Successfully requested confirmation!");
        } catch (RestApiException exception) {
            System.err.println("Error confirming delivery reports. " + exception.getMessage());
